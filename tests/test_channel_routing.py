...
def test_routes_live_strategy_to_live_channel() -> None:
    ...
    assert (
        route_channel("BAR-001", registry, sandbox=SANDBOX, live=LIVE, logging_channel_id=LOGGING)
        == LIVE
    )
...
def test_routes_sandbox_strategy_to_sandbox_channel() -> None:
    ...
    assert (
        route_channel("FOO-001", registry, sandbox=SANDBOX, live=LIVE, logging_channel_id=LOGGING)
        == SANDBOX
    )
...
def test_unknown_strategy_defaults_to_sandbox_channel() -> None:
    ...
    assert (
        route_channel("GHOST-001", registry, sandbox=SANDBOX, live=LIVE, logging_channel_id=LOGGING)
        == SANDBOX
    )
...
def test_unrecognized_mode_defaults_to_sandbox_channel() -> None:
    ...
    assert (
        route_channel("WEIRD-001", registry, sandbox=SANDBOX, live=LIVE, logging_channel_id=LOGGING)
        == SANDBOX
    )
...
def test_unscoped_topic_routes_to_logging_channel() -> None:
    ...
    assert (
        route_channel("", registry, sandbox=SANDBOX, live=LIVE, logging_channel_id=LOGGING)
        == LOGGING
    )
