"""Unit tests — ``tinohelm.factor.evaluation.distribution``.
...
* Mixed statistics conventions — ``np.std`` uses ddof=0 (population),
  ``pd.Series.skew``/``kurtosis`` use ddof=1 (sample, adjusted=True).  This
  mixed convention is baked into the legacy output and must not drift.
...
class TestShortCircuit:
    def test_empty_series_returns_empty(self):
        assert compute_distribution(pd.Series([], dtype=float)) == EMPTY

    def test_fewer_than_10_finite_returns_empty(self):
        # 9 finite values → short-circuit.
        assert compute_distribution(pd.Series([1.0] * 9)) == EMPTY

    def test_exactly_10_finite_passes_the_gate(self):
        out = compute_distribution(pd.Series(np.linspace(0, 1, 10)))
        assert out["histogram"] != []
        assert out["stats"] != {}

    def test_nan_and_inf_rows_do_not_count_toward_the_10_floor(self):
        vals = [np.nan] * 20 + [1.0] * 5 + [np.inf] * 10
        # Only 5 finite values → below 10 → empty.
        assert compute_distribution(pd.Series(vals)) == EMPTY


class TestSchema:
    def test_histogram_entry_keys(self):
        out = compute_distribution(pd.Series(np.linspace(-1, 1, 200)), n_bins=5)
    ...
    def test_histogram_has_exactly_n_bins(self):
        out = compute_distribution(pd.Series(np.linspace(-1, 1, 200)), n_bins=20)
        assert len(out["histogram"]) == 20

    def test_stats_has_all_nine_keys(self):
        out = compute_distribution(pd.Series(np.linspace(-1, 1, 200)))
    ...
    def test_count_sum_equals_finite_input_size(self):
        series = pd.Series(np.linspace(-1, 1, 300))
...
class TestFiltering:
    def test_nan_inf_filtered_from_histogram(self):
        ...
        poisoned = np.concatenate([[np.nan, np.inf, -np.inf], finite])
        out = compute_distribution(pd.Series(poisoned), n_bins=5)
    ...
    def test_poison_does_not_leak_into_stats(self):
        ...
        poisoned = np.concatenate([[np.nan, np.inf, -np.inf, np.nan], finite])
        out = compute_distribution(pd.Series(poisoned))
...
class TestPrecisionRounding:
    def test_mean_std_min_max_at_6dp(self):
        ...
        out = compute_distribution(pd.Series(rng.normal(0, 1, 1000)))
        stats = out["stats"]
    ...
    def test_bin_edges_at_6dp(self):
        ...
        out = compute_distribution(pd.Series(rng.normal(0, 1, 200)), n_bins=10)
        for entry in out["histogram"]:
...
class TestStatisticsConventions:
    def test_std_uses_ddof_zero_population(self):
        ...
        out = compute_distribution(pd.Series(values))
        assert out["stats"]["std"] == 1.0

    def test_skew_uses_pandas_adjusted_sample_convention(self):
        # pandas skew of [1, 2, 3, 10] with adjust=True matches the known value.
        # Pad to 10 elements for the gate.
        data = [1.0, 2.0, 3.0, 10.0, 1.0, 2.0, 3.0, 10.0, 1.0, 2.0]
        expected_skew = round(pd.Series(data).skew(), 4)
        out = compute_distribution(pd.Series(data))
    ...
    def test_autocorr_on_constant_series_coerces_nan_to_zero(self):
        # A constant series has zero variance → pd.autocorr returns NaN;
...
class TestKnownDistributions:
    def test_uniform_histogram_is_roughly_flat(self):
        ...
        out = compute_distribution(pd.Series(rng.uniform(0, 1, 10000)), n_bins=10)
