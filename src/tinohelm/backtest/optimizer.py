...
Pure numeric helpers live in :mod:`tinohelm.backtest.optimizer_helpers` so
they can be unit-tested without ``optuna`` or NautilusTrader installed.
"""
...
# Backward-compatible alias (external imports may reference the leading
# underscore form).  Prefer :data:`FAIL_VALUE` in new code.
...
def _create_sampler(name: str) -> Any:
    ...
    Optuna-dependent, so it lives here rather than in ``optimizer_helpers``.
    """
...
class BacktestOptimizer:
    ...
    def run(self) -> None:
        ...
        if self.patience <= 0:
            auto_p = auto_patience(self.n_trials)
            if auto_p > 0:
                self.patience = auto_p
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            try:
                r.publish(
                    ...
                    json.dumps(build_progress_event(
        ...
        if use_walk_forward and best_params:
            ...
            for fold_idx, (tr_s, tr_e, te_s, te_e) in enumerate(wf_windows):
                ...
                wf_fold_results.append(build_wf_fold_result(
                    fold_idx, tr_s, tr_e, te_s, te_e, fold_value,
                ))
        ...
        if (
            self.fitness_objective in DSR_COMPATIBLE_OBJECTIVES
            and validation_result
        ):
            try:
                val_stats = validation_result.get("statistics", {}) or {}
