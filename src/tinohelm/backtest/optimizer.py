...
Pure bookkeeping / math helpers live in
:mod:`tinohelm.backtest.optimizer_helpers` (NT-free *and* Optuna-free, so
they can be unit-tested in isolation).  This module retains the
Optuna-dependent orchestration: sampler instantiation, trial objectives,
DB persistence, and the study event loop.
"""
...
# Legacy aliases (retained for any external importer that reached into the
# private API before the helpers module was extracted).  New code should
# import from ``optimizer_helpers`` directly.
_FAIL_VALUE: float = FAIL_VALUE
...
# ---------------------------------------------------------------------------
# Sampler factory (Optuna-dependent, stays in this module)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Early-stopping callback (thin Optuna adapter over PatienceTracker)
# ---------------------------------------------------------------------------

class _PatienceCallback:
    """Optuna callback that stops the study after ``patience`` trials with
    no improvement to the best value.

    The thread-safe bookkeeping lives in
    :class:`tinohelm.backtest.optimizer_helpers.PatienceTracker`; this
    adapter just bridges Optuna's callback contract into
    :meth:`PatienceTracker.observe` and calls ``study.stop()`` when
    convergence stalls.
    """
