...
Pure helpers (date math, fitness extraction, smart defaults, statistics, payload
assembly) live in :mod:`tinohelm.backtest.optimizer_helpers` so they can be
unit-tested without Optuna / NT / Redis / SQLAlchemy.  This module keeps the
orchestration: Optuna study setup, Redis publishing, DB persistence, and the
shared backtest engine for trial reuse.
"""
...
# Backwards-compat aliases — older callers and tests may still import the
# private names.  New code should import from optimizer_helpers directly.
...
# ---------------------------------------------------------------------------
# Optuna sampler factory (kept here because it touches optuna types)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Optuna early-stopping callback (thin shim over PatienceTracker)
# ---------------------------------------------------------------------------
...
class BacktestOptimizer:
    ...
    - Parameter importance / sensitivity / stability analysis
    """
    ...
    def run(self) -> None:
        ...
        wf_windows: list[tuple[date, date, date, date]] = []
        if use_walk_forward:
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            # Track best (lock-free: Optuna calls objective serially with
            # n_jobs=1; for n_jobs>1 the GIL protects these simple
        ...
        # --- DSR (only for Sharpe objective) ---
        dsr_value: float | None = None
        if self.fitness_objective == "sharpe" and validation_result:
            try:
                from tinohelm.backtest.result import _norm_cdf, _norm_ppf
